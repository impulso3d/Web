...
$query = 'SELECT * FROM LU_CLIENTE';
$resultProd = $conexion->query($query);
$query = 'SELECT * FROM LU_TIPO_CLIENTE';
$resultTipoCli = $conexion->query($query);
?>

<h3>FORMULARIO DE INSERCION DE CLIENTE NUEVO</h3>
<form method="post" action="insercionCliente.php">

Cliente:<input name="cliente" type="text"/>
<br>
Tipo Cliente:<select name="tipoCli">    
    <?php    
    while ( $row = $resultTipoCli->fetch_array() )    
    {
        ?>
    
        <option value="<?php echo $row['id_tipo_cliente'] ?>" >
...
<font color="red"><b>CHEQUEAR SI EL CLIENTE NO ESTA YA EN LA BASE: </b></font><select name="consulta">    
    <?php    
