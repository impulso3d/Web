...
$query = 'SELECT * FROM LU_PRODUCTO';
$resultProd = $conexion->query($query);
$query = 'SELECT * FROM LU_TIPO_PRODUCTO';
$resultTipoProd = $conexion->query($query);
?>

<h3>FORMULARIO DE INSERCION DE PRODUCTO NUEVO</h3>
<form method="post" action="insercionProducto.php">

Producto:<input name="producto" type="text"/>
<br>
<input type='hidden' name="contrasena" value="<?php echo$contrasena; ?>" />  
Tipo Producto:<select name="tipoProd">    
    <?php    
    while ( $row = $resultTipoProd->fetch_array() )    
    {
        ?>
    
        <option value="<?php echo $row['id_tipo_producto'] ?>" >
...
<font color="red"><b>CHEQUEAR SI EL PRODUCTO NO ESTA YA EN LA BASE: </b></font><select name="consulta">    
    <?php    
...
<br><br><button onclick="history.go(-1);">VOLVER</button>
